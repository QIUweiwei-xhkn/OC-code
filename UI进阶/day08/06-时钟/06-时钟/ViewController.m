...
//    NSCalendarUnit type = NSCalendarUnitSecond|NSCalendarUnitMinute| NSCalendarUnitHour;
    // components：月历的组件，年，月，日，时，分，秒。
    // fromDate：从什么时间开始获取
    ...
    // 获得当前多少分
    NSInteger curM = cmp.minute;
    // 获得当前多少时
    NSInteger curH = cmp.hour;
    ...
    // 计算分针当前旋转的角度
    // angle = 当前多少秒 * 每秒多少度
    CGFloat minA = curM * perMinA;
    self.minL.transform = CATransform3DMakeRotation(angleToRad(minA), 0, 0, 1);
    // 时针开始旋转
    // 计算时针当前旋转的角度
    // angle = 当前多少秒 * 每秒多少度
    CGFloat hourA = curH * perHourA + curM * perMinHour;
...
- (void)setHour {
    ...
    hourL.bounds = CGRectMake(0, 0, 3 , 55);
